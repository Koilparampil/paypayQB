...
PAYEE_NAME      = os.getenv("PAYEE_NAME") if os.getenv("PAYEE_NAME") is not None else ""
PAYMENT_ACCOUNT = os.getenv("PAYMENT_ACCOUNT") if os.getenv("PAYMENT_ACCOUNT") is not None else ""
CSV_NAME        = os.getenv("CSV_NAME") if os.getenv("CSV_NAME") is not None else ""
...
def dismiss_cookie_banner(page: Page) -> None:
    for label in ("Allow all", "Accept all", "Accept cookies", "Accept"):
        try:
...
def wait_for_invoice_table(page: Page, urlIG: str) -> None:
    """Wait until the invoice grid is on screen, allowing time for a manual
    login (accounts.maersk.com redirect) on the way."""
    log(f"Opening Maersk ...")
    page.goto(urlIG, wait_until="domcontentloaded")
    dismiss_cookie_banner(page)

    log("Waiting... (log in manually if prompted)")
    deadline = datetime.now().timestamp() + LOGIN_TIMEOUT_MS / 1000
    while True:
        try:
            # nth(1) = first data row (nth(0) is usually the header row)
            page.get_by_role("row").nth(1).wait_for(timeout=10_000)
            return
        except PWTimeout:
            if datetime.now().timestamp() > deadline:
                raise RuntimeError(
                    "Timed out waiting for the invoice table. "
                    "Log in and re-run, or increase LOGIN_TIMEOUT_MS."
                )
            # After login Maersk sometimes lands on the dashboard instead of
            # the deep link — steer back to the invoices page.
            if "myfinance" not in page.url:
                try:
                    page.goto(urlIG, wait_until="domcontentloaded")
                except Exception:
                    pass
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        print(filled_inv_nums)
        page2 = context.new_page()
        wait_for_invoice_table(page2, f"https://www.maersk.com/payments-and-checkout/payment?ids={inv_num_string}&redirect=https://www.maersk.com/myfinance&platform=myfinance")
        print(f"{'─'*50}")
